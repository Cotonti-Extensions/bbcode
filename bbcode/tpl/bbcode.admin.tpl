...
<div class="block">
	...
	<form action="{ADMIN_BBCODE_UPDATE_URL}" method="post">
		<table class="cells">
			<tr>
				<td class="coltop width35">{PHP.L.Name} / {PHP.L.adm_bbcodes_mode} / {PHP.L.Enabled}
					/ {PHP.L.adm_bbcodes_container}</td>
				<td class="coltop width20">{PHP.L.adm_bbcodes_pattern}</td>
				<td class="coltop width20">{PHP.L.adm_bbcodes_replacement}</td>
				<td class="coltop width15">{PHP.L.Plugin} / {PHP.L.adm_bbcodes_priority}
					/ {PHP.L.adm_bbcodes_postrender}</td>
			...
			<tr>
				...
				<td class="centerall">
					<input type="button" value="{PHP.L.Delete}"
						   onclick="if(confirm('{PHP.L.adm_bbcodes_confirm}')) location.href='{ADMIN_BBCODE_ROW_DELETE_URL}'"/>
				</td>
			</tr>
...
<div class="block">
    ...
    <div class="wrapper">
        <ul>
            <li><a href="{ADMIN_BBCODE_URL_CLEAR_CACHE}"
                   onclick="return confirm('{PHP.L.adm_bbcodes_clearcache_confirm}')">{PHP.L.adm_bbcodes_clearcache}</a>
            </li>
            <!-- BEGIN: ADMIN_BBCODE_CONVERT -->
            <li><a href="{ADMIN_BBCODE_CONVERT_URL}"
                   onclick="return confirm('{PHP.L.adm_bbcodes_convert_confirm}')">{ADMIN_BBCODE_CONVERT_TITLE}</a></li>
